...
AngularApp.prototype.loadContainerConfig = function () {
    try {
        console.log('Getting config file: ' + chalk.cyan('config/' + this.env + '.json'));

        return JSON.stringify(require(process.cwd() + '/config/' + this.env + '.json'));
    } catch (exception) {
        console.warn(chalk.yellow('┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓'));
        console.warn(chalk.yellow("┃ ") + chalk.yellow('⚠  ') + chalk.red('WARNING!') + " No config file is present." + chalk.yellow("                ┃"));
...
AngularApp.prototype.angularTree = function () {
    var angular = new Funnel('bower_components/angular/', {
        ...
        shim = new Funnel(__dirname, {
            include: ['angular-shim.js']
        });

    return mergeTrees([angular, shim]);
};
...
AngularApp.prototype.addonsTree = function () {
    ...
    for (var dep in bowerConfig.dependencies) {
        if (dep.substr(0, 3) === 'rs-') {
            addons.push(new Funnel('bower_components/' + dep + '/src/app/' + dep, {
                destDir: 'addons/' + dep
            }));
        }
    }
...
AngularApp.prototype.processVendorScripts = function () {
    ...
    vendorScripts = concat(vendorScripts, {
        ...
        headerFiles : ['scripts/loader.js', 'angular.js', 'angular-shim.js'],
        inputFiles  : ['**/*.js'],
...
AngularApp.prototype.runLinter = function (appScriptsTree, appStylesTree) {
    var appScripts   = new Funnel(appScriptsTree, {
            exclude: ['addons/**/*', 'scripts/templates.*']
        }),
...
AngularApp.prototype.runTests = function (appScriptsTree, vendorScriptsTree, lintingResults) {
    var tests,
        unitTestingResults;
    ...
    unitTestingResults = unitTest(mergeTrees([vendorScriptsTree, appScriptsTree, tests, lintingResults]), {
...
AngularApp.prototype.toTree = function () {
    ...
    testResults = this.runTests(appScripts, vendorScripts, lintingResults);
    ...
    return mergeTrees(testResults.concat([html, appScripts, vendorScripts, appStyles, vendorStyles, appAssets]), { overwrite: true });
};
