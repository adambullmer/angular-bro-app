...
`;
    ...
    config = this.babel(config, { sourceRoot: 'config' });
...
AngularApp.prototype.babelPolyfillTree = function () {
    let babelPath = require.resolve('babel-polyfill');

    babelPath = babelPath.replace(/lib\/index\.js$/, 'dist');
...
AngularApp.prototype.processAngularTemplates = function (templates) {
    const templatesCache = ngTemplate([templates], {
...
AngularApp.prototype.babel = function (tree, options) {
    return new Babel(tree, Object.assign({
        ...
        plugins: [ "transform-es2015-modules-amd", "angularjs-annotate" ],
